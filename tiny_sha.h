...
bool SHA1Update(SHA1_CTX *ctx, const uint8_t *data, size_t len)
{
    ...
    while (len > 0)
    {
        ...
        if (ctx->num == 64)
        {
            if (!SHA1ProcessBlock(ctx, ctx->buf))
                return false;
            ctx->num = 0;
        }
    }
...
bool SHA256Update(SHA256_CTX *ctx, const uint8_t *data, size_t len)
{
    ...
    if (ctx->buf_len)
    {
        ...
        {
            if (!SHA256ProcessBlock(ctx, ctx->buf))
                return false;
            ctx->buf_len = 0;
        }
    }
    ...
    for (; i + SHA256_BLOCK_SIZE <= len; i += SHA256_BLOCK_SIZE)
        if (!SHA256ProcessBlock(ctx, data + i))
            return false;
...
bool SHA256Final(SHA256_CTX *ctx, uint8_t digest[SHA256_DIGEST_SIZE])
{
    ...
    // Process final block
    if (!SHA256ProcessBlock(ctx, block))
        return false;
...
bool SHA224Final(SHA224_CTX *ctx, uint8_t digest[SHA224_DIGEST_SIZE])
{
    ...
    if (!SHA256Final((SHA256_CTX *)ctx, full_digest))
        return false;
...
bool SHA512Update(SHA512_CTX *ctx, const uint8_t *data, size_t len)
{
    ...
    if (ctx->buf_len && (ctx->buf_len + len >= 128))
    {
        ...
        memcpy(ctx->buf + ctx->buf_len, data, fill);
        if (!SHA512ProcessBlock(ctx, ctx->buf))
            return false;
        ctx->buf_len = 0;
        i = fill;
    }
    ...
    for (; i + 127 < len; i += 128)
        if (!SHA512ProcessBlock(ctx, data + i))
            return false;
...
bool SHA512Final(SHA512_CTX *ctx, uint8_t digest[SHA512_DIGEST_SIZE])
{
    ...
    // update with padding
    if (!SHA512Update(ctx, pad, pad_len))
        return false;

    // update with length
    if (!SHA512Update(ctx, len_bytes, 16))
        return false;
...
bool SHA384Final(SHA384_CTX *ctx, uint8_t digest[SHA384_DIGEST_SIZE])
{
    ...
    if (!SHA512Final((SHA512_CTX *)ctx, full_digest))
        return false;
...
bool keccakP(uint64_t state[5][5], unsigned int w, unsigned int nr)
{
    if (!state || nr > KECCAK_ROUNDS || (w != 64 && w != 32))
        return false;
...
{
    if (ctx->finalized)
        return false; // Cannot absorb after finalization
...
{
    KECCAK_CTX ctx;
    return k_init_wrap(&ctx, rate, suffix) && k_absorb_wrap(&ctx, data, len) && k_final_wrap(&ctx) && k_squeeze_wrap(&ctx, digest, outlen);
}
